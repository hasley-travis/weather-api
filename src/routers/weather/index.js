...
// const streets = require('../../data/streets.js');

const log = bunyan.createLogger({ name: 'WeatherApp/routers/weather/index.js' });
const router = express.Router({ mergeRoutes: true });

const weatherRoute = (secrets) => {
  /**
  ...
  /**
   * Makes a call to MapQuest's geocoder API to obtain latitude and longitude
  ...
  async function getWeatherData(lat, lng) {
    ...
    url.searchParams.set('APPID', secrets[config.openWeatherMap.key]);
    ...
    log.info(`Calling getWeatherData function for lat, lng ${lat}, ${lng}`);
    return axios(options);
  }
  ...
  async function getWeatherHandler(req, res) {
    ...
    try {
      ...
      if (weatherResponse.status !== 200) {
        log.error(`Error calling getWeatherData: ${weatherResponse.statusText}`);
      ...
      log.error(`Error calling getWeatherHandler: ${e.message}`);
